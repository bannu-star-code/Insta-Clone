...
class ProfileEditView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        form = self.form_class(instance=request.user) # instance=request.user -> to pass current user saved details in edit form
        context = {'form':form}
        return render(request,self.template_name,context=context)

    def post(self,request,*args,**kwargs):
        form = self.form_class(request.POST,request.FILES, instance=request.user)
        # to handle file fields we have to pass request.FILES
        if form.is_valid():
            form.save()
...
class AllProfilesView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        # username__contains  -> lookup-fields
